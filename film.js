...
detail.querySelector('#premiera').innerHTML = 
	`Premiéra <strong>${premieraText}</strong>, ${kdy}`


let uzivatelovyHvezdy = 0
const hvezdicky = detail.querySelectorAll('.stars .fa-star')

function zvyrazniHvezdy(zvyraznitPocet) {
	hvezdicky.forEach((hvezdicka, hvezdaCislo) => {
		if (hvezdaCislo < zvyraznitPocet) {
...
hvezdicky.forEach((hvezdicka, index) => {
	hvezdicka.addEventListener('click', () => {
		uzivatelovyHvezdy = index+1
		zvyrazniHvezdy(uzivatelovyHvezdy)
	})

	hvezdicka.addEventListener('mouseenter', () => {
		zvyrazniHvezdy(index+1)
	})
